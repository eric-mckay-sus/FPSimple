...
public class ConsoleInputProvider : IInputProvider
{
    ...
    public async Task<bool> GetConfirmAsync(Report prompt)
    {
        ...
        string response = (await GetInputAsync(new(string.Empty))).Trim().ToLower();
...
public class ConsoleReporter : IReportOutputProvider
{
    ...
    public async Task ShowPreview(DataTable dt)
    {
        ...
        if(dt.Rows.Count == 0){
            await ReportAsync(new("\tNo rows with valid data (under current filters).\n", ReportLevel.WARNING));
            return;
        }
        ...
        await ReportAsync(new(sb.ToString()));
    }
}
