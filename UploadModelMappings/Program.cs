...
public sealed class ModelInfoMap : ClassMap<ModelInfo>
{
    public ModelInfoMap()
    {
        Map(m => m.IcsNum).Name("INTERNAL_PART_#");
        Map(m => m.ShortDescription).Name("SHORT_DESC");
        Map(m => m.ProdCellCode).Name("PROD_CELL_CODE");
        Map(m => m.WorkCenterCode).Name("WORK_CENTER_CODE");
